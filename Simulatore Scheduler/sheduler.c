...
int durata = 0; //Durata della simulazione in termini di job da creare(es: 50 job)
...
void* produttore(void* arg) {
    ...
    while (1) {
        ...
        usleep(SLEEP_TIME); //aspetta un decimo di secondo e dopo ricomincia a creare 
    }
    return NULL;
}
...
void* consumatore(void* arg) {
    ...
    while (1) {
        ...
        while (counter == 0 && creati<durata) { //se ho la coda vuota ma puo' ancora aggiornata
            pthread_mutex_unlock(&lock);
            sched_yield();
            pthread_mutex_lock(&lock);
        }
        if (counter==0 && creati == durata) { //se ho la coda vuota e non posso piu' aggiungere job 
            pthread_mutex_unlock(&lock);
            break;
        }
        ...
        Process *p = dequeue(q);
        counter--;
        pthread_mutex_unlock(&lock); // devo simulare il run di un processo, di conseguenza posso/devo rilasciare (perche' conveniente risorse globali)

        //SIMULAZIONE ESECUZIONE JOB
        gettimeofday(&(p->start), NULL); // salvo l'orario in cui inizia a essere eseguito
        usleep(p->exec_time); // simula l'esecuzione
        gettimeofday(&(p->end), NULL); // salvo l'orario in cui finisce di essere eseguito
        // calcolo effettivo T_end e T_arrival(turnatoundtime = T_end - T_arrival)
